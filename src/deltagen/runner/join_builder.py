...
def apply_single_join(
    ...
    for cond in join_config.conditions:
        F = _get_spark_functions()
        # Resolve left side: supports bare column names, "alias.column" notation,
        # and SQL expressions (detected by presence of parentheses, e.g. "LOWER(ifa)").
        if "(" in cond.left:
            left_col = F.expr(cond.left)
        elif "." in cond.left:
        ...
        # Resolve right side: supports bare column names and SQL expressions.
        right_col_name = renamed.get(cond.right, cond.right)
        if "(" in cond.right:
            right_col = F.expr(cond.right)
        else:
